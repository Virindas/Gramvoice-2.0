...
const adminNav = [
  ...
  { to: "/admin/services", label: "Service Requests", icon: HandHelping },
  { to: "/admin/rules", label: "Manage Rules", icon: Scale },
  { to: "/admin/announcements", label: "Announcements", icon: Megaphone },
  { to: "/admin/contacts", label: "Directory Contacts", icon: Phone },
...
function SideNav({ portal }: { portal: "citizen" | "admin" }) {
  ...
  const items = rawItems.map((item) => ({
    ...
    translatedLabel:
      ...
      item.to.includes("rulebook") ? t("dash.info.rules") :
      item.to.includes("contacts") ? t("dash.hub.contacts") :
...
function BottomNav({ portal }: { portal: "citizen" | "admin" }) {
  ...
  const citizenBottomNav = [
    { to: "/citizen/home", label: t("nav.dashboard"), icon: Home },
    { to: "/citizen/complaint/new", label: t("tileRaiseComplaint"), icon: FilePlus2 },
    { to: "/citizen/complaints", label: t("tileMyComplaints"), icon: ListChecks },
    { to: "/citizen/services", label: t("dash.btn.services"), icon: HandHelping },
    { to: "/citizen/profile", label: t("nav.profile"), icon: User },
  ];
  const items = portal === "citizen" ? citizenBottomNav : adminNav.map(i => ({
    ...i,
    label: i.to.includes("profile") ? t("nav.profile") : i.label
  }));
  const { pathname } = useLocation();
  return (
    <nav
...
export function TopBar({
  ...
  const { pathname } = useLocation();
  const showLangSwitcher = pathname.endsWith("/home") || pathname.endsWith("/profile") || pathname === "/";
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <h1 className="truncate text-xl font-black uppercase tracking-wider text-foreground lg:text-2xl">{title}</h1>
...
export function Shell({
  ...
  const authed = portal === "citizen" ? (!!citizen || !!getUser("citizen")) : (!!admin || !!getUser("admin"));

  useEffect(() => {
    if (hydrated && !authed) {
      const loginRoute = portal === "citizen" ? "/citizen/login" : "/admin/login";
