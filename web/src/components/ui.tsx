...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  { label, error, hint, className, rightElement, ...rest },
  ref,
) {
  const uid = useId();
  return (
...
export function StatusBadge({ status, className }: { status: Status; className?: string }) {
  ...
  const translatedStatus = 
    status === "Under Review" ? t("status.review") :
    status === "In Progress" ? t("status.progress") :
    status === "Completed" ? t("status.resolved") :
...
export function Alert({ tone, children, className }: { tone: "success" | "error" | "info" | "warning"; children: ReactNode; className?: string }) {
  const tones = {
  ...
  return (
    <div
      role="status"
      className={cn("flex items-start gap-2 rounded-xl px-4 py-3 text-base font-medium", tones[tone], className)}
    >
      {tone === "success" ? (
...
export function LanguageSwitcher({ className }: { className?: string }) {
  const { db } = useStore();
  const api = useApi();
  const { language, setLanguage: setI18nLanguage } = useLanguage();
  const currentLangLabel = language === "hi" ? "Hindi" : language === "ta" ? "Tamil" : "English";
  ...
  return (
    ...
    >
      ...
      <select
        ...
        onChange={(e) => {
          const selected = e.target.value as (typeof LANGUAGES)[number];
          api.setLanguage(selected);
          const langCode: LanguageCode = selected === "Hindi" ? "hi" : selected === "Tamil" ? "ta" : "en";
