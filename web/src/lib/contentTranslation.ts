...
// Catalog of verified translations for mock/seed data & common grievances
...
// Memory cache for runtime translations
const runtimeTranslationCache: Record<string, string> = {};

/**
 * Translates a category string to the target language.
 */
...
/**
 * Translates a contact role string.
 */
...
/**
 * Translates a service request type.
 */
...
/**
 * Synchronously translates rule details if available.
 */
...
/**
 * Synchronously translates announcement details if available.
 */
...
/**
 * Hook to translate complaint text (title, body, reply, category)
 * dynamically into the current active language (Hindi or Tamil).
 */
export function useTranslatedComplaint(complaint?: Complaint | null): TranslatedContent {
  ...
  useEffect(() => {
    ...
    // Check pre-translated fields from DB
    ...
    // Check catalog of seed complaints
    ...
    // For dynamic tickets, request translation from backend
    if (complaint.id) {
      let isMounted = true;
      setTranslated((prev) => ({
        ...prev,
